...
func NewWal(path string) (*Wal, error) {
	...
	if err != nil {
		return nil, pkgerrors.WalError{Path: path, Err: errors.Join(pkgerrors.WalFailedToCreateError, err)}
	}
...
func (w *Wal) WriteToWal(actionString string) (n int, err error) {
	...
	if err != nil {
		err = pkgerrors.WalError{Path: w.path, Err: errors.Join(pkgerrors.WalFailedToWriteError, err, w.CloseWal())}
		return 0, err
	}
...
func (w *Wal) RecoverFromWal(applyFunc func(line string) error) error {
	...
	if _, err := w.file.Seek(0, 0); err != nil {
		return pkgerrors.WalError{Path: w.path, Err: errors.Join(pkgerrors.WalFailedToReadError, err)}
	}
...
func (w *Wal) CloseWal() error {
	if err := w.file.Close(); err != nil {
		return pkgerrors.WalError{Path: w.path, Err: errors.Join(pkgerrors.WalFailedToCloseError, err)}
	}
	return nil
}
...
func (w *Wal) SyncWal() error {
	if err := w.file.Sync(); err != nil {
		return pkgerrors.WalError{Path: w.path, Err: errors.Join(pkgerrors.WalFailedToSyncError, err)}
	}
	return nil
}
...
func (w *Wal) TruncateWal() error {
	if err := os.Truncate(w.path, 0); err != nil {
		return pkgerrors.WalError{Path: w.path, Err: errors.Join(pkgerrors.WalFailedToTruncateError, err)}
	}
	return nil
}
